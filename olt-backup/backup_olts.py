...
com envio automático dos arquivos de backup para o Telegram.
...
def enviar_backups_telegram():
    ...
    if enviados:
        telegram_send_message(
            f"✅ {enviados} arquivo(s) de backup enviado(s) com sucesso."
        )
...
def run_backups():
    """Executa o backup de todas as OLTs e envia notificação ao Telegram."""
    olts = get_olts()
    log.info("=" * 60)
    log.info("Iniciando backups de %d OLTs...", len(olts))
    log.info("=" * 60)
    ...
    resumo = f"📋 Resumo do backup OLT — {ts}\n\n"
    resumo += f"✅ Sucesso ({len(resultados_ok)}): {', '.join(resultados_ok) if resultados_ok else 'nenhum'}\n"
    resumo += f"❌ Falha ({len(resultados_erro)}): {', '.join(resultados_erro) if resultados_erro else 'nenhum'}\n"
